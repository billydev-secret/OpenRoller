...
- A server can set a minimum game time with `/risky_set_min_game_time`, counted from when the round opened. While it runs, nothing closes the round: not **Close Round**, not the player-count auto-close, and not a minutes auto-close set shorter than the minimum, which waits for it instead. It applies equally to the opener and to admins, and is skipped entirely for rounds started with `/risky_start_no_ping`.
- Servers that have never set a minimum have none on **Close Round** — whoever opened the round can close it as soon as two people have rolled. The automatic close still waits out `DEFAULT_MIN_GAME_SECONDS` (30 minutes by default, set by whoever hosts the bot) so a round doesn't end the instant the last expected player rolls.
...
- **A minimum game time of 0 now means 0, not "disabled".** Older versions stored no value at all for `/risky_set_min_game_time 0`; this version stores `0` itself. Nothing to do either way: a stored `0` and a server that never configured one both leave **Close Round** unrestricted, exactly as before.
